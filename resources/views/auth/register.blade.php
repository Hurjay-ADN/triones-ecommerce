	...
	<body>
		<section>
			<div class="flex justify-center items-center h-screen">
				<form action="{{ route('auth.store') }}" class="border rounded border-gray-300 w-full lg:w-4/12 p-3" method="POST">
					@csrf
					<div>
					...
					<div>
						<button class="block my-3 w-full rounded py-1 px-3 bg-blue-600 text-white cursor-pointer">Register</button>
						<div class="flex justify-center">
							<span class="text-sm inline-block">Have an account? <a href="{{ route('login') }}" class="hover:text-blue-600 text-blue-400">Login here</a> </span>
						</div>
					</div>
				</form>
			</div>
